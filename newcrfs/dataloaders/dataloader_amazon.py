...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            depth_gt = Image.fromarray(np.array(Image.open(depth_path)).astype("uint16")) 


            image = image.resize(new_size)
            depth_gt = depth_gt.resize(new_size)
            ...
            depth_gt = np.array(depth_gt)
            valid_mask = np.zeros_like(depth_gt)
            conveyor = np.max(depth_gt)
            valid_mask[depth_gt < conveyor] =1
            depth_gt[valid_mask==0] = 0
            depth_gt = Image.fromarray(depth_gt)
